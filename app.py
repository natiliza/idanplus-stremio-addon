...
ADDON_NAME = os.getenv("ADDON_NAME", "Idan+ Live")
        ...
        with self._lock:
            ...
            try:
                ...
                if resp.status_code == 304:
                    log.info("channels.json not modified")
                elif resp.ok:
                    data = resp.json()
                    if isinstance(data, dict) and data:
                        ...
                        log.info("channels.json refreshed from remote (%d channels)", len(data))
                else:
                    log.warning("remote refresh failed: %s", resp.status_code)
            except Exception as exc:
                log.warning("remote refresh error: %s", exc)
...
def make_id(channel_id: str) -> str:
    return f"idanplus:{channel_id}"


def parse_meta_id(meta_id: str) -> str:
    if not meta_id.startswith("idanplus:"):
        raise ValueError("unsupported id")
    return meta_id.split(":", 1)[1]
...
def build_meta_preview(channel: dict) -> dict:
    ...
    poster = f"{PUBLIC_BASE_URL}/assets/{image}" if image else f"{PUBLIC_BASE_URL}/assets/icon.png"
    description = (
        f"{channel_type_label(kind)} • מודול: {channel.get('module', 'tv')} • "
        f"מזהה: {channel.get('channelID', '')}"
    )
    return {
        "id": make_id(channel["channelID"]),
        "type": "tv",
...
def get_i24_token() -> Optional[str]:
    ...
    auth_url = (
        f"https://api.i24news.wiztivi.io/authenticate?userName=I24News"
        f"&hardwareId={hardware_id}&hardwareIdType=browser"
    )
...
def resolve_14tv(channel: dict) -> ResolvedStream:
    ...
    if ch_api:
        resp = SESSION.get(
            ch_api,
            headers={"x-tenant-id": "channel14", "User-Agent": DEFAULT_UA},
...
def resolve_keshet(channel: dict) -> ResolvedStream:
    ...
    entitlements = (
        ...
        f"?et=ngt&lp={raw_path}&rv=AKAMAI"
    )
...
def resolve_reshet(channel: dict) -> ResolvedStream:
...
def build_streams(channel: dict) -> dict:
    try:
            ...
            "streams": [
                {
                    ...
                    "description": str(exc),
                    "externalUrl": channel.get("linkDetails", {}).get("referer")
                    or channel.get("linkDetails", {}).get("link")
                    or PUBLIC_BASE_URL,
                }
            ]
        }
    ...
    def _dispatch(self) -> None:
        ...
        if path == "/manifest.json":
            manifest = {
                ...
                "version": "0.1.0",
                "name": ADDON_NAME,
                "description": "Live TV and radio from Idan+ Kodi sources with hot-reloaded channels.json backend.",
                "logo": f"{PUBLIC_BASE_URL}/assets/icon.png",
                "background": f"{PUBLIC_BASE_URL}/assets/icon.png",
                "resources": [
                    "catalog",
                    {"name": "meta", "types": ["tv"], "idPrefixes": ["idanplus:"]},
                    "stream",
                ],
                "types": ["tv"],
                "catalogs": [
                    {"type": "tv", "id": "idanplus_live", "name": "Idan+ Live TV"},
                    {"type": "tv", "id": "idanplus_radio", "name": "Idan+ Radio"},
                ],
        ...
        if m:
            ...
            if catalog_id == "idanplus_live":
                metas = [build_meta_preview(ch) for ch in get_channels("tv")]
                return response_json(self, {"metas": metas})
            if catalog_id == "idanplus_radio":
                metas = [build_meta_preview(ch) for ch in get_channels("radio")]
                return response_json(self, {"metas": metas})
        ...
        if m:
            meta_id = m.group(1)
            try:
                channel_id = parse_meta_id(meta_id)
            except ValueError:
        ...
        if m:
            ...
            if not channel:
                return response_json(self, {"streams": []}, 404)
            return response_json(self, build_streams(channel))
        ...
        if path == "/":
            return response_json(
                self,
                {
                    ...
                    "tv": len(get_channels('tv')),
                    "radio": len(get_channels('radio')),
                },
            )
