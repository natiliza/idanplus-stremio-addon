...
MAX_META_VIDEOS = int(os.getenv("MAX_META_VIDEOS", "80"))
MAX_RECURSION_DEPTH = int(os.getenv("MAX_RECURSION_DEPTH", "3"))
CACHE_TTL_SEC = int(os.getenv("IDANPLUS_CACHE_TTL", "300"))
CACHE_MAX_ENTRIES = int(os.getenv("IDANPLUS_CACHE_MAX_ENTRIES", "128"))
PREBUILD_TTL_SEC = int(os.getenv("IDANPLUS_PREBUILD_TTL", "1800"))
PREBUILD_STARTUP = os.getenv("IDANPLUS_PREBUILD_STARTUP", "true").lower() == "true"
PREBUILD_SYNC_FALLBACK = os.getenv("IDANPLUS_PREBUILD_SYNC_FALLBACK", "false").lower() == "true"
PREBUILD_TARGET_NAMES = {name.strip() for name in os.getenv("IDANPLUS_PREBUILD_NAMES", "כאן,קשת / מאקו,רשת 13,עכשיו 14").split(",") if name.strip()}
...
class TTLCache:
    def __init__(self, ttl_sec: int = 300, max_entries: int = 128) -> None:
...
BRIDGE_CACHE = TTLCache(ttl_sec=CACHE_TTL_SEC, max_entries=CACHE_MAX_ENTRIES)
VIDEOS_CACHE = TTLCache(ttl_sec=CACHE_TTL_SEC, max_entries=CACHE_MAX_ENTRIES)
PREBUILD_STATE_LOCK = threading.Lock()
PREBUILD_STATE: Dict[str, Dict[str, Any]] = {}
...
def get_root_tv_entries() -> List[MenuEntry]:
    base_entries: List[MenuEntry] = [
        MenuEntry(module="__section__", name="ערוצי טלוויזיה", url="idanplus_live", mode=0, icon="live.jpg", catalog_id="idanplus_tv"),
    ]
    for vod_entry in SECTION_ROOTS.get("idanplus_vod", []):
        base_entries.append(clone_entry(vod_entry, name=f"VOD - {vod_entry.name}", catalog_id="idanplus_tv"))
    base_entries.extend([
...
def _entry_cache_key(entry: MenuEntry) -> str:
    return hashlib.sha1(
        f"videos|{entry.module}|{entry.mode}|{entry.url}|{entry.more_data}|{MAX_META_VIDEOS}|{MAX_RECURSION_DEPTH}".encode("utf-8")
    ).hexdigest()


def _provider_base_name(entry: MenuEntry) -> str:
    return entry.name.replace("VOD - ", "", 1) if entry.name.startswith("VOD - ") else entry.name


def _is_prebuild_target(entry: MenuEntry) -> bool:
    return entry.catalog_id == "idanplus_tv" and entry.name.startswith("VOD - ") and _provider_base_name(entry) in PREBUILD_TARGET_NAMES


def _set_prebuild_state(entry: MenuEntry, status: str, message: str = "") -> None:
    with PREBUILD_STATE_LOCK:
        PREBUILD_STATE[_entry_cache_key(entry)] = {"status": status, "message": message, "updated": time.time()}


def _get_prebuild_state(entry: MenuEntry) -> Dict[str, Any]:
    with PREBUILD_STATE_LOCK:
        return dict(PREBUILD_STATE.get(_entry_cache_key(entry), {}))


def _warm_entry(entry: MenuEntry) -> None:
    cache_key = _entry_cache_key(entry)
    _set_prebuild_state(entry, "warming")
    try:
        videos = collect_videos(entry)
        VIDEOS_CACHE.set(cache_key, videos)
        _set_prebuild_state(entry, "ready", f"{len(videos)} פריטים")
    except Exception as exc:
        log.warning("prebuild failed for %s: %s", entry.name, exc)
        _set_prebuild_state(entry, "error", str(exc))


def _prebuild_loop() -> None:
    while True:
        try:
            for entry in get_root_tv_entries():
                if _is_prebuild_target(entry):
                    _warm_entry(entry)
        except Exception as exc:
            log.warning("prebuild loop error: %s", exc)
        time.sleep(max(300, PREBUILD_TTL_SEC))


def start_prebuild_worker() -> None:
    if not PREBUILD_STARTUP:
        return
    thread = threading.Thread(target=_prebuild_loop, name="idanplus-prebuild", daemon=True)
    thread.start()
...
class ModuleBridge:
    ...
    def run(self, entry: MenuEntry) -> Dict[str, object]:
        cache_key = hashlib.sha1(f"bridge|{entry.module}|{entry.mode}|{entry.url}|{entry.more_data}".encode("utf-8")).hexdigest()
        cached = BRIDGE_CACHE.get(cache_key)
        ...
        try:
            ...
            result = common.end_capture()
        except Exception as exc:
            log.warning("module bridge failed for %s %s: %s", entry.module, entry.mode, exc)
            result = {"items": [], "stream": {"error": str(exc)}}
        BRIDGE_CACHE.set(cache_key, result)
        return result
...
def synthetic_children(entry: MenuEntry) -> Optional[List[MenuEntry]]:
    ...
    if section_id == "idanplus_tv":
        return [clone_entry(item) for item in get_root_tv_entries()]
...
def list_catalog_entries(catalog_id: str) -> List[MenuEntry]:
    if catalog_id == "idanplus_tv":
        return [clone_entry(entry) for entry in get_root_tv_entries()]
...
def build_channel_meta(entry: MenuEntry) -> dict:
    ...
    cache_key = _entry_cache_key(entry)
    videos = VIDEOS_CACHE.get(cache_key)

    if videos is None and _is_prebuild_target(entry):
        state = _get_prebuild_state(entry)
        if not state:
            _set_prebuild_state(entry, "queued")
            threading.Thread(target=_warm_entry, args=(entry,), name=f"warm-{_provider_base_name(entry)}", daemon=True).start()
        elif state.get("status") not in {"warming", "queued"}:
            threading.Thread(target=_warm_entry, args=(entry,), name=f"rewarm-{_provider_base_name(entry)}", daemon=True).start()
        if PREBUILD_SYNC_FALLBACK:
            videos = collect_videos(entry)
            VIDEOS_CACHE.set(cache_key, videos)
        else:
            videos = []
            state = _get_prebuild_state(entry)
            meta["description"] = state.get("message") or "הקטלוג נטען ברקע. נסה שוב בעוד כמה שניות."
            meta["releaseInfo"] = "טוען ברקע"
            meta["videos"] = []
            return {"meta": meta}
    ...
    if videos:
        ...
        state = _get_prebuild_state(entry)
        if _is_prebuild_target(entry) and state.get("status") in {"warming", "queued"}:
            meta["releaseInfo"] = "טוען ברקע"
            meta["description"] = state.get("message") or "הקטלוג נטען ברקע. נסה שוב בעוד כמה שניות."
        else:
    ...
    def _dispatch(self) -> None:
        ...
        if path == "/manifest.json":
            manifest = {
                ...
                "version": "0.5.0",
                "name": ADDON_NAME,
                "description": "Idan+ fast TV hub with background-prebuilt VOD catalogs for faster provider opening.",
