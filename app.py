...
class MenuEntry:
    ...
    url_params_data: Optional[Dict[str, Any]] = None
...
ROOT_TV_ENTRIES: List[MenuEntry] = [
    MenuEntry(module="__section__", name="ערוצי טלוויזיה", url="idanplus_live", mode=0, icon="live.jpg", catalog_id="idanplus_tv"),
    MenuEntry(module="__section__", name="VOD", url="idanplus_vod", mode=0, icon="vod.jpg", catalog_id="idanplus_tv"),
...
def _normalize_item(item: dict, catalog_id: str) -> Optional[MenuEntry]:
    ...
    raw_is_playable = bool(item.get("isPlayable", False))
    raw_is_folder = bool(item.get("isFolder", True))
    # Do not guess playability from mode alone: in the original Kodi addon,
    # several folder-like items use the same numeric modes as playable items.
    # Guessing here turns program folders into fake playable videos and Stremio
    # then shows "No streams" for almost everything.
    is_playable = raw_is_playable
    is_folder = raw_is_folder and not is_playable
    return MenuEntry(
        ...
        url_params_data=item.get("urlParamsData") or {},
    )
...
def synthetic_children(entry: MenuEntry) -> Optional[List[MenuEntry]]:
    ...
    if section_id == "idanplus_tv":
        return [clone_entry(item) for item in ROOT_TV_ENTRIES]
...
def list_catalog_entries(catalog_id: str) -> List[MenuEntry]:
    if catalog_id == "idanplus_tv":
        return [clone_entry(entry) for entry in ROOT_TV_ENTRIES]
